  ...
  async function generatePDF() {
    ...
    images.value.forEach((image, index) => {
      ...
      if (index === images.value.length - 1) {
        ...
        formData.append("attachment", new Blob([doc.output('blob')], { type: "application/pdf" }), computedFilename.value);
        formData.append("user", session.user?.id || "");
        const createdRecord = $pb.records.create('exams', formData).then((record) => {
          console.log(record);
        });
      } else {
        doc.addPage();
